...
const DocumentQAPage = () => {
  ...
  // NEW: Collapsible PDF pane state
  const [isPdfCollapsed, setIsPdfCollapsed] = useState(false);
  const togglePdfCollapse = useCallback(() => setIsPdfCollapsed(s => !s), []);
  ...
  return (
    <div className="qa-page-layout" style={{ position: 'relative' }}>
      ...
      <main className="qa-container">
        {/* LEFT: PDF / preview pane (collapsible) */}
        <div
          className={`qa-left pdf-viewer ${isPdfCollapsed ? 'collapsed' : ''}`}
          style={{
            flex: isPdfCollapsed ? '0 0 68px' : '0 0 50%',
            width: isPdfCollapsed ? '68px' : '50%',
            transition: 'width 220ms ease, flex 220ms ease'
          }}
        >
          ...
          <button
            ...
            title={isPdfCollapsed ? 'Expand preview' : 'Collapse preview'}
            aria-label={isPdfCollapsed ? 'Expand preview' : 'Collapse preview'}
          >
            <svg
              width="20"
              height="28"
              ...
              style={{
                display: 'block',
                transform: isPdfCollapsed ? 'rotate(180deg)' : 'rotate(0deg)',
          ...
          {!isPdfCollapsed ? (
            ...
            /* collapsed minimal view: show filename vertically */
            <div style={{
              display: 'flex',
              alignItems: 'center',
      ...
      <style>{`
        /* Container: tighter padding and clearer gap */
        .qa-page-layout { min-height: 100vh; display:flex; position:relative; background:#0b0f14; }
        .qa-container { width:100%; display:flex; gap: 20px; padding: 24px 36px; position:relative; z-index:1; align-items:stretch; }

        /* LEFT pane (preview) - start at exactly 50% */
        .qa-left {
        ...
        /* Collapsed left: small fixed width */
        .qa-left.collapsed {
        ...
        /* When left is collapsed, make right pane fill remaining space */
        .qa-left.collapsed + .qa-right {
          flex: 1 1 auto;
          width: auto;
          max-width: none;
        }

        /* Collapse toggle button: slightly larger pill with crisp chevron */
        .pdf-collapse-btn {
          position: absolute;
          top: 50%;
          right: -16px;
        ...
        .pdf-collapse-btn:hover { transform: translateY(-50%) translateX(3px); background: rgba(255,255,255,0.07); }
        .pdf-collapse-btn svg { color: currentColor; width: 18px; height: 24px; }

        /* RIGHT pane (chat) - start at exactly 50% */
        .qa-right {
          flex: 0 0 50%;
          max-width: none;
          min-width: 0;
        ...
        /* Ensure preview content hidden when collapsed and keep a neat collapsed visual */
        .qa-left.collapsed .pdf-scroll,
        .qa-left.collapsed .docx-preview,
        .qa-left.collapsed .image-preview,
        .qa-left.collapsed iframe {
          display: none;
        }

        /* Keep PDF scroll and iframe responsive when expanded */
        ...
        /* Small responsive touch: allow chat to shrink on narrow screens */
        @media (max-width: 1100px) {
          .qa-right { flex: 0 0 44%; min-width: 300px; }
          .qa-left { min-width: 220px; }
        }
        @media (max-width: 800px) {
          .qa-container { padding: 16px; gap: 12px; flex-direction: column; }
          .qa-left, .qa-right { width: 100%; flex: none; max-width: 100%; min-width: 0; }
          .pdf-collapse-btn { display: none; }
        }

        /* Make the "Ask your document" header black */
        .qa-header {
          background: #000;
          padding: 10px 12px;
          border-radius: 10px;
